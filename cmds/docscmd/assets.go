...
3. 执行本地 %sgit commit%s；
4. 执行 %sgit push%s；
5. 报告结果。

除非用户明确要求不要推送，否则**默认执行 %sgit push%s**。

## 必读上下文

优先按以下顺序读取或获取：

- %sgit diff --cached --stat%s
- %sgit diff --cached --name-only%s
- 如有必要，再查看 %sgit diff --cached%s

如果 staged diff 为空，不要立即结束；继续读取：

- %sgit status --short%s
- %sgit diff --stat%s
- %sgit diff --name-only%s
- 如有必要，再查看 %sgit diff%s
...
1. 如果 staged diff 非空：直接执行 %sgit commit -m "<best>"%s。
2. 如果 staged diff 为空但 unstaged diff 非空：先执行 %sgit add -A%s，再执行 %sgit commit -m "<best>"%s。
3. 提交成功后，默认继续执行 %sgit push%s。
4. 优先推送当前分支的上游；如果没有上游，则推送当前分支到同名远程分支。
5. 如果 %sgit commit%s 失败，应输出失败原因，而不是假装成功。
6. 如果 %sgit push%s 失败，应输出真实失败原因。
...
1. **用户可见新能力** 优先于内部重构细节。
   - 例如：新增命令、子命令、交互入口、脚手架、repo prompt、工作流能力，优先考虑 %sfeat%s。
2. **真实行为修复** 优先于实现细节调整。
   - 例如：修复发布流程、修复输出错误、修复空发布，优先考虑 %sfix%s。
3. **纯结构调整且无新增用户能力** 才优先考虑 %srefactor%s。
4. **纯文档更新** 才优先考虑 %sdocs%s。

如果一次改动同时包含“用户可见新能力”和“内部重构”，应优先围绕**最核心的用户可见变化**选择 %stype%s。
...
2. 使用 **Conventional Commits** 规范：
   ...
   - %sci%s
3. 标题格式：
   - %s<type>(<scope>): <summary>%s
   - 如果 scope 不明确，可省略 scope，使用 %s<type>: <summary>%s。
4. %ssummary%s 使用英文，简洁明确，尽量不超过 50 个字符。
5. 优先描述本次改动的**核心行为变化**，不要机械罗列所有文件。
6. 如果主要是新增能力，优先用 %sfeat%s。
7. 如果主要是修复问题，优先用 %sfix%s。
8. 如果主要是无行为变化的结构调整，优先用 %srefactor%s。
9. 如果主要是文档、说明、注释更新，优先用 %sdocs%s。
10. 如果同时存在代码和文档改动，优先按代码主行为决定 type。
11. 如果改动新增了命令、子命令、prompt、规则文件、脚手架或发布工作流，且这些内容对用户直接可见，优先考虑 %sfeat%s，不要轻易降级成 %srefactor%s。
...
- %smode:%s 说明本次基于 %sstaged%s 还是 %sunstaged-auto-stage%s
- %scommit:%s 实际执行的 commit message
- %shash:%s 实际生成的 commit hash（短 hash 即可）
- %spush:%s 推送目标或推送结果摘要
- %sreason:%s 用中文简短说明为什么这条提交信息最合适（1~2 句）
...
3. 顶层字段固定使用：
   ...
   - %spush:%s
   - %sreason:%s
4. %scommit:%s 必须是**单行** commit message。
5. %shash:%s 必须来自真实 %sgit commit%s 结果。
6. %spush:%s 必须来自真实 %sgit push%s 结果摘要。
7. %sreason:%s 只写 1~2 句中文，简洁即可。

如果当前没有任何可用改动，请只输出一段简短提示，说明：

- 当前没有 staged diff
- 当前也没有 unstaged diff（如果确实为空）
- 请先修改代码或执行 %sgit add%s
